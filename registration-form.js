/* 
    ...
    Version: 4.0
    Description: Javascript for MIS 7375 Homework 4 - Patient Registration Form. Validate data on the fly from a form
*/
...
reviewButton.addEventListener("click", function () {
  console.log("reviewButton clicked");
  showReview();
});
...
document.querySelectorAll(".formElement").forEach((element) => {
  if (element.name && element.id) {
    ...
    element.addEventListener("focus", () => validateElement(true));
    element.addEventListener("blur", () => validateElement(false));
  }
});
...
function showReviewValidation() {
  ...
  const reviewRows = getFormElements().map(
